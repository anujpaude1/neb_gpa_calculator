<script>
  ...
  function update_gpap(event) {
    ...
    const selected_class = event.target.className
      .split(" ")[0]
      .toString()
      .trim();
  ...
  function calculate_gpa(event) {
    ...
    let marks_to_gpa_result = marks_to_gpa(
      selected_class,
      parseInt(classarr[1]),
      selected_value,
    );
  ...
  function marks_to_gpa(subject_name, position, value) {
    ...
    if (
      (90 / 100) * subjects[subject_name][position] > value &&
      value >= (80 / 100) * subjects[subject_name][position]
    ) {
      return 3.6;
    }

    if (
      (80 / 100) * subjects[subject_name][position] > value &&
      value >= (70 / 100) * subjects[subject_name][position]
    ) {
      return 3.2;
    }
    if (
      (70 / 100) * subjects[subject_name][position] > value &&
      value >= (60 / 100) * subjects[subject_name][position]
    ) {
      return 2.8;
    }
    if (
      (60 / 100) * subjects[subject_name][position] > value &&
      value >= (50 / 100) * subjects[subject_name][position]
    ) {
      return 2.4;
    }
    if (
      (50 / 100) * subjects[subject_name][position] > value &&
      value >= (40 / 100) * subjects[subject_name][position]
    ) {
      return 2;
    }
    if (
      (40 / 100) * subjects[subject_name][position] > value &&
      value >= (35 / 100) * subjects[subject_name][position]
    ) {
      return 1.6;
    }
    if (value === "") {
...
<div id="input_method">
  ...
  <div class="buttons">
    <button
      class={is_active1 ? "active" : "inactive"}
      on:click={() => select_input("gpa")}
    >
      GPA
    </button>
    <button
      class={is_active2 ? "active" : "inactive"}
      on:click={() => select_input("marks")}>Marks</button
    >
  </div>
</div>
...
{#if input_type === "gpa"}
  <h2>
    <i style="font-weight:100"
      >Please only select the GPA for subjects that you have taken.</i
    >
  </h2>
  <div id="changing">
...
{:else if input_type === "marks"}
  <h2>
    <i style="font-weight:100"
      >Please enter marks for only subjects that you have taken.</i
    >
  </h2>
  <div id="changing">
    {#each Object.keys(subjects) as subject}
  ...
  {#if always_true === true}
    {#each Object.keys(subjects) as subject}
      ...
      {:else if subjects[subject][0] !== 0 && subjects[subject][1] !== 0}
        <h2 style="color:rgb(34, 163, 23);">
          {subject} : {subjects[subject][6]}
        </h2>
      {/if}
    {/each}
  {/if}
